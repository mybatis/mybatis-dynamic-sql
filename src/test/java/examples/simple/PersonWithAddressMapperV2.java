 ...
 */
@Mapper
public interface PersonWithAddressMapperV2 extends CommonCountMapper {
    ...
    default Optional<PersonWithAddress> selectOne(SelectDSLCompleter completer) {
        SelectDSL start = SelectDSL.select(selectList).from(person)
    ...
    default long count(CountDSLCompleter completer) {
        CountDSL start = CountDSL.countFrom(person)
                .join(address, on(person.addressId, isEqualTo(address.id))).endJoin();
