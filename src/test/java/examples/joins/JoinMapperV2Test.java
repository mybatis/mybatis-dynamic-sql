...
class JoinMapperV2Test {
    ...
    void testMultipleTableJoinWithWhereClause() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = SelectDSL.select(orderMaster.orderId, orderDate, orderLine.lineNumber, itemMaster.description, orderLine.quantity)
                    .from(orderMaster, "om")
    ...
    void testRightJoin2() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = SelectDSL.select(orderLine.orderId, orderLine.quantity, itemMaster.itemId, itemMaster.description)
                    .from(orderMaster, "om")
                    .join(orderLine, "ol").on(orderMaster.orderId, isEqualTo(orderLine.orderId))
    ...
    void testFullJoin() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = SelectDSL.select(orderLine.orderId, orderLine.quantity, orderLine.itemId.as("ol_itemid"), itemMaster.itemId.as("im_itemid"), itemMaster.description)
                    .from(itemMaster, "im")
    ...
    void testFullJoin4() {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            SelectStatementProvider selectStatement = SelectDSL.select(orderLine.orderId, orderLine.quantity, itemMaster.description)
                    .from(orderMaster, "om")
                    .join(orderLine, "ol", on(orderMaster.orderId, isEqualTo(orderLine.orderId)))
                    .fullJoin(itemMaster, "im", on(orderLine.itemId, isEqualTo(itemMaster.itemId)))
